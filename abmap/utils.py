...
def get_boolean_mask(sequence, chain_type, scheme, buffer_region, dev, fold=0,
    ...
    temp_name = os.path.join(anarci_dir, sequence[:250])
    ...
    try:
        if chain_type == 'H':
            file_name = glob.glob(os.path.join(anarci_dir, f'*{sequence[:250]}_H.csv'))[0]
        else:
            file_name = glob.glob(os.path.join(anarci_dir, f'*{sequence[:250]}_KL.csv'))[0]
